...
@php
    ...
    $canDownloadLeadDocuments = $isSuperAdminUser || (session('user_role') !== 'team_manager' && $access->can('documents.download'));
...
.lead-doc-upload input[type=file]{
    max-width:100%;
    font-size:12px;
}
.lead-doc-replace{
...
<div class="lead-workbench-v2">
    ...
    <div class="card lead-mobile-section" data-mobile-section="more" style="margin:0 0 18px;">
        ...
        @if ($canWorkThisLead)
            <form method="POST"
                ...
                <input type="hidden" name="return_to" value="{{ url('/leads/' . $lead->id) }}?lead_tab=more#communication-documents">
                <input type="file" name="file" required>
                <button type="submit" class="btn-small btn-primary">📤 Upload evidence</button>
            </form>
        @endif

        <div id="communication-documents" class="lead-doc-list">
                    ...
                    <div class="lead-doc-actions">
                        ...
                        @if ($canWorkThisLead)
                            ...
                            <form method="POST"
                                ...
                                @csrf
                                <input type="hidden" name="return_to" value="{{ url('/leads/' . $lead->id) }}?lead_tab=more#communication-documents">
                                <button type="submit" class="btn-small" style="background:var(--c-danger);color:#fff;">Remove</button>
                            </form>
                            @endif
                        @endif
                    </div>
            ...
            <div class="lead-doc-panel">
                        ...
                        <div class="lead-doc-actions">
                            ...
                            @if ($canWorkThisLead)
                                <form method="POST"
                                      ...
                                      class="lead-doc-replace">
                                    @csrf
                                    <input type="hidden" name="return_to" value="{{ url('/leads/' . $lead->id) }}?lead_tab=more#media-{{ $bookingDocument->id }}">
                                    <input type="file" name="file" required>
                                    <button type="submit" class="btn-small">Replace</button>
                                </form>
                            @endif
                ...
                @elseif ($canWorkThisLead)
                    <form method="POST"
                        ...
                        <input type="hidden" name="return_to" value="{{ url('/leads/' . $lead->id) }}?lead_tab=more#booking-documents">
                        <input type="file" name="file" required>
...
(function(){
    function initLeadWorkbench(){
        ...
        var first=requestedTab
            ? document.querySelector('details[data-accordion="'+requestedTab+'"]')
            : document.querySelector('details[data-accordion="history"]');

        if(first&&!first.open) first.open=true;

        if(requestedTab){
            document.querySelectorAll('[data-lead-wb-section]').forEach(function(button){
                button.classList.toggle(
                    'is-active',
                    button.getAttribute('data-lead-wb-section')===requestedTab
                );
            });
        }
    ...
    function restoreLeadReturnFocus(){
        ...
        if(requestedTab){
            var accordion=document.querySelector(
                'details[data-accordion="'+requestedTab+'"]'
            );
