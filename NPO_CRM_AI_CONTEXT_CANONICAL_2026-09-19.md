...
## Production Hotfix 2026-09-20 — Timeline Intelligence site-visit date + customer-information action source
- Live production issue reported: Lead workflow could return HTTP 500 from `TimelineIntelligenceService::signals()` because `DB::table('site_visits')->get()` returns `visit_at` as a string, while the service called `toIso8601String()` as if it were a Carbon instance.
- Fix: parse the database value explicitly with `Carbon::parse((string) $latestVisit->visit_at)` before formatting. No site-visit data or schema is changed.
- Live customer-information save could return SQLSTATE[22001]/1406 because `activities.action_source` was VARCHAR(20), while the controlled source keys `customer_conversation` and `customer_confirmation` are 21 characters.
- Fix: expand `activities.action_source` to VARCHAR(50). The customer-information narrative remains in `notes`; it is not moved into `action_source` and no note truncation is introduced.
- Manual live SQL is supplied because this CRM's business schema is maintained through phpMyAdmin and must not be assumed to be fully migration-managed. A forward migration is also included for baseline consistency.
- No authorization changes, no notification changes, no Lead/Customer business-rule changes, and no new route.
- Prepared for deployment only; not considered live until the user confirms deployment and verifies both workflows.

## 2026-09-20 — Customer Information Evidence Isolation Hotfix
- Production verification after the Customer Information Display hotfix found partial display contamination: saved budget/preferred area/criteria could appear correctly, while possession and purchase/decision timeline could be replaced by values extracted from unrelated historical text; objections could be malformed because broad regex matched substrings such as `objection` inside `Concerns / objections`.
- Root cause: Timeline Intelligence mixed the controlled `customer_conversation` evidence with broad historical extraction. The Customer Information form writes canonical labelled lines, but the intelligence layer could still select unrelated timeline text for the same requirement fields.
- Fix: controlled `customer_conversation` entries are now a separate authoritative evidence source for Customer Information fields: budget, preferred area, possession requirement, purchase/decision timeline, decision criteria, and concerns/objections.
- Customer Information reconstruction is deterministic and newest-first; activity ordering now uses `logged_at DESC, id DESC` to remove same-timestamp ambiguity.
- Broad extraction no longer runs against `customer_conversation` activities for controlled fields. This prevents false matches such as `buy` inside `Buyer Type` and malformed extraction from `Concerns / objections`.
- The Lead Customer panel therefore displays the exact saved controlled values under **ALREADY KNOWN**, and the update form repopulates those same saved values. Previously saved timeline evidence is reused; no data migration or timeline rewrite is required.
- No database schema change, route change, permission change, project/lead business-data mutation, or task workflow change.

## Release 2026-09-20 — Contact Pitch Project / Reassignment v1 (PREPARED, NOT DEPLOYED)

### Objective
Formalize the real operational Contact-list model: whenever a contact list is circulated, it carries one **Pitch Project** that the caller is expected to pitch. The Pitch Project is not the customer's permanent project and does not constrain the eventual Lead project.

### Implemented in this prepared build
- Added `contact_project_assignments` as the historical assignment layer.
- A Contact retains one current pitch context through the existing `contacts.project_id` / `assigned_to_agent_id` compatibility fields; assignment history is preserved in the new table.
- Reassignment closes the prior active assignment and creates a new active assignment for the new caller/project. The same Contact can therefore move from Project A / Caller A to Project B / Caller B without duplicating the Contact identity.
- DND (`status = dnc`) is a server-side circulation gate: DND contacts cannot be assigned/reassigned or called and cannot be promoted to a Lead.
- Contact import now requires a list-level Pitch Project and can assign the imported list to an authorized caller. Agent imports default to the importing agent; Team Manager/Admin scopes are enforced server-side.
- Contact promotion now requires an explicit **Lead Project** selection. The Lead project is independent of the Pitch Project, so a caller can pitch Project A and create a genuine enquiry as a Lead for Project B.
- Contact assignment history is visible on the Contact page. Agent call history remains restricted to the current agent; management scopes retain broader history visibility.
- First logged call marks the current active pitch assignment as started; it does not alter assignment history.
- Existing Contact search/dialer semantics continue to use the current assignment fields, while DND is additionally enforced server-side.

### Database
- New table SQL: `database/sql/CONTACT_PITCH_PROJECT_ASSIGNMENTS_2026_09_20.sql`
- Laravel migration: `database/migrations/2026_09_20_080000_create_contact_project_assignments_table.php`
- No migration is assumed to have run in production. This release remains **PREPARED, NOT DEPLOYED** until the user explicitly confirms database SQL + application deployment.

### Release discipline
- Deployment package contains only the modified existing PHP/Blade files plus the new model, migration, and SQL installation script.
- Rollback package contains exact pre-release versions of every modified existing file; new files are removed manually by the rollback instructions.
- Full updated CRM baseline is packaged from this prepared source tree.
- PHP syntax lint: 280 PHP files, 0 failures.


## Contact Pitch Project / Reassignment v1 — 2026-09-20 sample/conversion refinement (PREPARED — NOT DEPLOYED)

- Contact import uses a mandatory list-level Pitch Project selected from the user's authorized existing projects; the CSV does not carry a Project column.
- Contact → Lead conversion defaults Lead Project to the Contact's current Pitch Project.
- Caller may choose a different authorized Lead Project only when the customer's actual requirement differs from the pitched project.
- Pitch Project remains historical operational context; it does not lock the eventual Lead Project.
- Corrected downloadable contact samples (`contacts_sample.csv` and `contacts_advanced.csv`) to match the list-level Pitch Project model.
- Status: PREPARED — NOT DEPLOYED.

## 2026-09-20 — Flexible Contact Import Wizard (Prepared, Not Deployed)

The next Contact import build is prepared but not yet deployed. It changes the import experience from a CRM-specific CSV format to a flexible source-file workflow:
- Accept CSV, XLS and XLSX uploads.
- For Excel files, parse the workbook in the browser with pinned SheetJS 0.20.3; let the user select a worksheet when there are multiple sheets; normalize the selected worksheet to CSV in memory; then send it through the existing server-side Contact import preview/execute pipeline.
- Show detected source columns and suggested mappings for Name, Phone, Email and Source; allow the user to change mappings before import.
- Name and Phone are mandatory at final import; Email and Source are optional.
- Do not require a Project column in the source file. Pitch Project remains a mandatory CRM-side selection from authorized existing Projects and applies to the whole imported list.
- Pitch Project remains the default Lead Project on Contact conversion, with explicit override to another authorized Project when the customer requirement differs.
- Reject duplicate source-column mappings and reject uploads over 20,000 data rows instead of silently truncating.
- Preserve existing duplicate handling, DND circulation gate, authorization scoping, ContactProjectAssignment history and Lead routing.

Status: **PREPARED — NOT DEPLOYED.**

## 2026-09-20 — Contact Assignment Authorization + Bulk Operations v1 (PREPARED, NOT DEPLOYED)
- Built against the deployed Contact Schema-Aware v2 + Dialer Super Admin + Log Call + Project Search baseline.
- The previously prepared Contact Pitch Project Agent Authorization fix is incorporated and remains NOT deployed until this combined release is explicitly confirmed.
- Contact Pitch Project → Caller assignment is now enforced twice: controller authorization and ContactService invariant. An Agent may receive a Contact for a Project only when the Agent is currently eligible for that Project through active direct project routing or active project-team membership.
- Contact import preview/execute now applies the same project-agent eligibility rule, preventing imports from creating an unauthorized Contact → Project → Caller combination.
- Contacts index gains checkbox selection, page Select All, Select All Matching, and authorized bulk actions: Assign/Change Pitch Project, Assign/Change Caller, and Clear Pitch Assignment.
- Bulk operations re-check Contact visibility, user scope, project scope, caller scope and project/caller eligibility server-side for every Contact. Invalid records are skipped with a reason rather than bypassing authorization.
- Added Super Admin-only Contact Assignment Audit/Reconciliation. It lists active assignments where the current caller is no longer eligible for the assigned project and can safely complete only the invalid assignment row; Contact, call, import and historical assignment data are preserved.
- No database schema change is introduced by this release. The existing `contact_project_assignments` history layer remains authoritative for assignment history.
- Deployment status: PREPARED ONLY. Do not treat this release as live until the user explicitly confirms deployment.

## 2026-09-21 — Caller Contact Follow-up Work System v1 — PREPARED, NOT DEPLOYED

### Product rule
The Contacts/Dialer surface is an execution surface for callers. A caller should not have to decide what to do next. The system presents unfinished work first, then due Contact retries, then fresh Contacts.

### Contact lifecycle before Lead conversion
- Caller opens Calling Work.
- Any call started through the CRM is recorded as an open `contact_call_session` until an outcome is logged.
- Open call sessions older than two minutes are surfaced as **Unfinished calls** and must be resolved before fresh work.
- Logging a Contact call automatically:
  - increments call attempts and connected count as before;
  - updates the Contact stage (`called`, `interested`, `not_interested`, `invalid`, `dnc` as applicable);
  - cancels the previous pending Contact retry;
  - creates the next Contact retry from the configured Call Outcome delay/action/priority when the Contact remains callable;
  - completes the open call session marker.
- Contact follow-ups are stored separately from Lead follow-ups because `/tasks` is Lead work only.
- Contact follow-ups are cancelled when the Contact is promoted to a Lead.
- Lead promotion continues to create the normal Lead first-contact follow-up, which then appears in `/tasks`.

### Caller opening experience
`/contacts/dialer` is now a work queue rather than a dashboard:
1. Unfinished calls first.
2. Contact follow-ups due today/overdue next.
3. Fresh Contacts only after the caller explicitly starts fresh work.
4. Completing a Contact call returns to the Calling Work queue so the next outstanding item is immediately visible.

The old large dialer banner/stat block and sequential blind queue were removed from the primary workflow. Session statistics are no longer allowed to compete with the work itself.

### Contacts list UX
- Mobile now receives a real Contact card list; the desktop table remains for larger screens.
- Filters are secondary and collapsed unless active.
- The top of Contacts points the caller directly to today's work when unfinished/due work exists.
- Converted Contacts explicitly display **Converted to Lead** and link directly to the Lead.

### Database
Prepared migration and manual SQL:
- `contact_followups` — Contact-only pending/done/cancelled retries.
- `contact_call_sessions` — records that the caller clicked Call but has not yet recorded an outcome.

No existing Contact, Lead, Call or Followup rows are modified by the migration itself.

### Deployment state
This release is PREPARED ONLY. It is NOT deployed or confirmed live. Production deployment requires applying the new database tables first, then uploading the prepared files, then browser verification of:
- call-start marker → unfinished-call recovery;
- outcome → Contact stage + next retry;
- Contact promotion → Contact retry cancellation + Lead follow-up in `/tasks`;
- mobile Contacts list;
- converted-to-lead label/link;
- desktop/mobile Calling Work queue navigation.
