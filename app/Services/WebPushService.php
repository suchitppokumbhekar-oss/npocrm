...
/**
 * Native Web Push sender for the NPO CRM PWA.
 *
 * Payload encryption follows the aes128gcm Web Push content coding
 * and VAPID authentication uses ES256.
 */
class WebPushService
{
    ...
    public function subscribe(int $userId, array $subscription): PushSubscription
    {
        ...
        if (
            ...
            ! in_array(parse_url($endpoint, PHP_URL_SCHEME), ['https'], true)
        ) {
    ...
    /**
     * Send one specific notification immediately to one device.
     *
     * This bypasses the per-device cursor used by the background worker,
     * allowing the manual test alert to be sent immediately.
     */
    public function sendNotification(
        ...
        $payload = [
            ...
            'body' => $notification->body
                ?: 'Open NPO CRM to review this notification.',
            'icon' => '/icons/icon-192.png',
            'badge' => '/icons/icon-192.png',
            'url' => $notification->action_url ?: '/notifications',
            'notification_id' => $notification->id,
            'priority' => NotificationService::priorityForType(
                $notification->type
            ),
        ];
        ...
        if ($ok) {
            $subscription->forceFill([
                'last_notification_id' => max(
                    (int) $subscription->last_notification_id,
                    (int) $notification->id
                ),
                'last_used_at' => now(),
                'last_error_at' => null,
    ...
    /**
     * Send newly-created notifications to a subscription.
     *
     * A small burst guard combines a large group into one visible push
     * so a busy CRM session cannot spam a user's device.
     */
        ...
        if ($notifications->count() > 3) {
            ...
            $payload = [
                ...
                'body' => $notifications->count()
                    . ' new notifications need your attention.',
                'icon' => '/icons/icon-192.png',
                'badge' => '/icons/icon-192.png',
                'url' => '/notifications',
    ...
    {
        $rows = PushSubscription::where('user_id', $userId)
        ...
        return [
            ...
            'healthy_count' => $rows
                ->filter(fn ($row) => ! $row->last_error_at)
                ->count(),
            'last_used_at' => optional(
                $rows->first()
            )->last_used_at?->toIso8601String(),
            'last_error_at' => optional(
                $rows->sortByDesc('last_error_at')->first()
            )->last_error_at?->toIso8601String(),
        ];
    }
    ...
    {
        $path = storage_path(
            'app/npo-crm-push-heartbeat.json'
        );
        ...
        if (! is_file($path)) {
            return [
                ...
                'sent' => 0,
                'subscriptions' => 0,
                'message' => 'Push worker has not reported a run yet.',
            ];
        }
        ...
        return is_array($data)
            ...
            : [
                ...
                'sent' => 0,
                'subscriptions' => 0,
                'message' => 'Invalid push worker heartbeat.',
            ];
    }
    ...
    private function send(
        ...
        try {
            ...
            if ($status < 200 || $status >= 300) {
                ...
                Log::warning(
                    'NPO CRM Web Push delivery failed',
                    [
                        'status' => $status,
    ...
    /**
     * Load the persistent VAPID key pair, generating it only when
     * this installation does not already have one.
     */
    private function keys(): array
    {
        ...
        if (
            ...
            throw new RuntimeException(
                'CRM storage directory is not writable.'
            );
        }
        ...
        if (
            ...
            throw new RuntimeException(
                'Unable to create Web Push VAPID keys.'
            );
        }
        ...
        if (
            ...
            throw new RuntimeException(
                'Unable to read Web Push VAPID public key.'
            );
        }
        ...
        if (
            ...
            throw new RuntimeException(
                'Unable to persist Web Push VAPID keys.'
            );
        }
    ...
    private function vapidJwt(string $audience): string
    {
        ...
        if (! $ok) {
            throw new RuntimeException(
                'Unable to sign Web Push VAPID token.'
            );
        }
    ...
    private function encryptPayload(
        ...
        $uaPublicRaw = $this->base64UrlDecode(
            $subscription->p256dh
        );

        $auth = $this->base64UrlDecode(
            $subscription->auth
        );

        if (
            strlen($uaPublicRaw) !== 65 ||
            $uaPublicRaw[0] !== "\x04" ||
            strlen($auth) < 16
        ) {
            throw new RuntimeException(
                'Invalid browser push encryption keys.'
            );
        }

        $auth = substr($auth, 0, 16);

        $uaPublicPem = $this->rawP256ToPem(
            $uaPublicRaw
        );
        ...
        if ($ephemeralPrivate === false) {
            throw new RuntimeException(
                'Unable to create push encryption key.'
            );
        }

        $ephemeralDetails = openssl_pkey_get_details(
            $ephemeralPrivate
        );

        $serverPublicRaw =
            "\x04"
            . $ephemeralDetails['ec']['x']
            . $ephemeralDetails['ec']['y'];
        ...
        if ($sharedSecret === false) {
            throw new RuntimeException(
                'Unable to derive Web Push shared secret.'
            );
        }
        ...
        $nonce = $this->hkdfExpand(
            $prk,
            "Content-Encoding: nonce\0",
            12
        );

        // RFC 8188: one record with a final-record delimiter.
        ...
        if ($ciphertext === false) {
            throw new RuntimeException(
                'Unable to encrypt Web Push payload.'
            );
        }
