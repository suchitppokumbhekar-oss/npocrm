...
class WorkflowPresentationService
{
    ...
    public const TYPE_FOLLOWUP_ACTION_TYPE = 'followup_action_type';
    ...
    public function assertActivityTypeAllowedForUser(?string $key, ?int $userId = null): void
    {
        $key = trim((string) $key);
        if ($key === '') {
            return;
        }
        $canonical = app(SettingsService::class)->activityTypeByKey($key);
        if (! $canonical) {
            throw new \DomainException('That activity type is not valid.');
        }
        $userId ??= (int) session('user_id');
        if ($userId <= 0) {
            return;
        }
        $row = UserWorkflowPresentation::query()->where('user_id', $userId)->where('option_type', self::TYPE_ACTIVITY_TYPE)->where('canonical_key', $key)->first();
        if ($row && ! $row->is_visible) {
            throw new \DomainException('That activity type is not available for your account.');
        }
    }

    public function assertFollowupActionTypeAllowedForUser(?string $key, ?int $userId = null): void
    {
        $key = trim((string) $key);
        if ($key === '') {
            return;
        }
        $canonical = app(SettingsService::class)->actionTypeByKey($key);
        if (! $canonical) {
            throw new \DomainException('That follow-up action type is not valid.');
        }
        $userId ??= (int) session('user_id');
        if ($userId <= 0) {
            return;
        }
        $row = UserWorkflowPresentation::query()->where('user_id', $userId)->where('option_type', self::TYPE_FOLLOWUP_ACTION_TYPE)->where('canonical_key', $key)->first();
        if ($row && ! $row->is_visible) {
            throw new \DomainException('That follow-up action type is not available for your account.');
        }
    }

    public function presentFollowupActionTypes(Collection $types, ?int $userId = null): Collection
    {
        $userId ??= (int) session('user_id');
        $overrides = $userId > 0 ? $this->presentationsForUser($userId, self::TYPE_FOLLOWUP_ACTION_TYPE) : collect();

        return $types->filter(function ($type) use ($overrides) {
            $override = $overrides->get($type->key);

            return ! $override || $override->is_visible;
        })->sortBy(function ($type) use ($overrides) {
            $override = $overrides->get($type->key);

            return [$override?->sort_order ?? (int) $type->sort_order, (int) $type->sort_order, $type->key];
        })->values()->map(function ($type) use ($overrides) {
            $override = $overrides->get($type->key);
            $type->presentation_label = trim((string) ($override?->display_label ?? '')) ?: $type->label;

            return $type;
        });
    }

    public function presentActivityTypes(Collection $types, ?int $userId = null): Collection
    {
        $userId ??= (int) session('user_id');
        $overrides = $userId > 0 ? $this->presentationsForUser($userId, self::TYPE_ACTIVITY_TYPE) : collect();
